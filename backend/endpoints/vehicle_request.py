...
from includes.connection_mysqli import get as connection, is_connected, cur_conn_close, cur_close, conn_close
...
def input_vehicles(value, name):
    ...
    if type(value) is dict:
        ...
        try: datetime.datetime.strptime(value["startDateTime"], "%Y-%m-%d %H:%M:%S.%f").date()
        except: raise ValueError("startDateTime not in correct format")
        try: datetime.datetime.strptime(value["endDateTime"], "%Y-%m-%d %H:%M:%S.%f").date()
        except: raise ValueError("endDateTime not in correct format")
...
class VehicleRequest(Resource):
  ...
  def get(self):
    ...
    if is_connected(conn):
      ...
      try:
        cur.execute("SELECT COUNT(`idRequest`) AS `count` FROM `asset-request_vehicle` AS arv WHERE arv.`idRequest`=%s;", [requestID])
        res = cur.fetchone()
  ...
  def post(self):
    ...
    conn_close(conn)
    return { "success": False }                    # Success Message